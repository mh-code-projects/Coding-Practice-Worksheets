...
function functionE(a, b, functionF){
  functionF(a, b);
};
...
function userFunction(user, functionA, functionB){
  if (user.name != null) {
    functionA(user);
  } else {
    functionB(user);
...
// userFunction(users[0], functionA, functionB);
...
function usersCallbackCaller(users, usersCallbackCalled){
  for (let i=0;i<users.length;i++){
    let user = users[i];
    usersCallbackCalled(user)
  }
}

usersCallbackCaller(users, deleteUser);
usersCallbackCaller(users, addNewUser);
